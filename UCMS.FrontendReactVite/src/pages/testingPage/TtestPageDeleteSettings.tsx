...
interface AdvancedSettingField { // Renamed for clarity
  id: string;
  label: string;
  description: string;
  typeValue: string;
...
interface SettingsData {
  ...
  advancedFields: AdvancedSettingField[];
}
...
interface SettingsPanelProps {
  ...
  onDeleteAdvancedField: (fieldId: string) => void; // New prop for deleting
}
const SettingsPanel: React.FC<SettingsPanelProps> = ({ data, onDataChange, onDeleteAdvancedField }) => (
  <div className="space-y-6">
    ...
    <div className="bg-gray-50 p-6 rounded-lg shadow"><h3 className="text-lg font-semibold text-gray-700 mb-1">Advanced Settings</h3><p className="text-xs text-gray-500 mb-4">Define field types (mimicking your image).</p>
      <div className="space-y-4">
        {data.advancedFields.map((field) => (
          <div key={field.id} className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto] gap-4 items-end"> {/* Adjusted grid for delete button */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Field: {field.label}</label>
              <p className="text-xs text-gray-500">{field.description}</p>
            </div>
            <div className="md:col-start-2"> {/* Ensure type input is in the second column on md+ */}
            ...
            <button
              onClick={() => onDeleteAdvancedField(field.id)}
...
// --- Main App Component ---
const TestPageDeleteSettings: React.FC = () => {
  ...
  const handleDeleteAdvancedField = (fieldIdToDelete: string) => {
    setCurrentPageData(prev => ({
      ...prev,
      settings: {
        ...prev.settings,
        advancedFields: prev.settings.advancedFields.filter(field => field.id !== fieldIdToDelete),
      },
    }));
  };
  ...
  const toggleChatPanel = () => setIsChatOpen(prev => !prev);
            ...
            {editorView === 'visual' ? (
              <>
                ...
                <div id="tab-panels">
                  ...
                  {activeVisualTab === 'settings' && <SettingsPanel data={currentPageData.settings} onDataChange={handleSettingsDataChange} onDeleteAdvancedField={handleDeleteAdvancedField} />}
                </div>
              </>
            ) : (
...
export default TestPageDeleteSettings;
