...
interface PageData {
  ...
  code: PageCodeData; // Added for HTML/CSS code
}
...
const initialPageData: Record<string, PageData> = {
  ...
  // Add more pages as needed, ensuring they also have a 'code' property
  about: {
    id: 'about',
    title: 'About Us',
    description: 'Editing the about us page.',
    content: { /* ... content data ... */ headerTitle: 'About Our Company', mainHeadline: 'Our Story', subtitle: 'Learn more about our journey.', bodyText: '<p>Details about the company...</p>', ctaButton: 'Meet the Team', currentImageUrl: 'https://placehold.co/600x300/A78BFA/5B21B6?text=About+Us' },
    media: [ /* ... media assets ... */ ],
    settings: { /* ... settings data ... */ pageSlug: 'about', seoTitle: 'About Us - MyCMS', seoDescription: 'Learn about MyCMS.', publishStatus: true, advancedFields: [] },
    code: {
      html: `<h1>About Us Page</h1>\n<p>Content specific to about page.</p>`,
      css: `h1 { color: purple; }`,
    }
  },
};
...
const TestPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Ensure the loaded page data has the 'code' property
    if (!pageDataToLoad.code) {
        pageDataToLoad.code = { html: placeholderHTML, css: placeholderCSS };
    }
    setCurrentPageData(pageDataToLoad);
    setActiveVisualTab('content'); // Reset visual tab when page changes
    // setEditorView('visual'); // Optionally reset to visual view when page changes
  }, [activePageId]);
  ...
  const handleSavePage = () => {
    console.log("Saving page:", activePageId, "Current View:", editorView, currentPageData);
    alert(`Page data (${editorView} view) saved to console!`);
  };
